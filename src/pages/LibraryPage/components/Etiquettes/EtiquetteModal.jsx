...
const EtiquetteModal = ({ isOpen, onClose, article, etiquettesHook }) => {
  ...
  const handleDeleteEtiquette = (etiquette) => {
    if (window.confirm(`Êtes-vous sûr de vouloir supprimer l'étiquette "${etiquette.nom}" ? Cette action est irréversible.`)) {
      console.log('🗑️ Suppression étiquette:', etiquette.nom, 'ID:', etiquette.id);
      supprimerEtiquette(etiquette.id);
      
      // Force un re-render des composants parents
      setTimeout(() => {
        console.log('✅ Étiquette supprimée, état mis à jour');
      }, 100);
    }
  };
